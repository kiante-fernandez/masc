...
- `duration_model`: Character. Method for modeling fixation durations:
  - “none” (default) - no duration modeling
  - “invgauss” - inverse Gaussian distributed durations
- `duration_params`: List. Parameters for the duration model:
  - For “invgauss”: nu (mean duration in ms, default: 250), lambda
    (shape parameter, default: 2.5)
...
- `results`: Data frame with trial-by-trial results including:
  ...
  - best option
  - choice consistency
  - number of fixations (rt_fixations)
  - total response time in ms (rt_ms)
  - fixation proportions
- `weights`: Vector of attribute weights used
- `parameters`: List of model parameters used
- `raw`: List containing detailed raw data for each trial including:
  - `trial`: Trial number
  ...
  - `fix_durations`: Vector of durations in ms
  - `total_rt_ms`: Total response time
  - `mean_fix_duration`: Mean fixation duration
